...
void GLViewNewModule::onKeyDown(const SDL_KeyboardEvent& key)
{
    ...
    {
        NvFlexSolverDesc mode;
        mode.featureMode = eNvFlexFeatureModeSimpleFluids;
    ...
    /*if (key.keysym.sym == SDLK_7)
    {
        ...
        float4 first;
        float4 second;
        SpawnParticles(&first, &second, &x, 10);
    }*/
}
...
/*void GLViewNewModule::createMessage() {
    std::cout << "This is my message\n";
    std::string reply;
    std::getline(std::cin, reply);

    Module4Message msg1(reply, this->getCamera()->getPosition());
    client->sendNetMsgSynchronousTCP(msg1);
}*/
...
/*int Aftr::GLViewNewModule::SpawnParticles(float4* particles, float4* velocities, int* phases, int numParticles) {
